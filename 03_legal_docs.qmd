...
Dans une première étape, nous extrayons les noms des aires protégées dans les textes des décrets.
...
Une fois les noms des aires protégées concernées extraites de chaque texte, on les compare aux noms de la base SAPM. On le fait tout d'abord automatiquement avec un algorithme d'appariement approximatif ("fuzzy matching"), qui tient compte des variations d'orthographe ou des dénominations multiple, afin d'identifier les correspondances les plus probables.
...
decision_pa <- split_class_clean %>%
  ...
  rename(verified_match = closest_match)  %>%
  distinct(pa, num_texte, .keep_all = TRUE) %>%
  select(date_texte = date_txt,
         ap_nom_texte = pa,
         ap_match_sapm = verified_match,
         texte = textes,
         type_texte = type,
         num_texte,
...
write_rds(decision_pa, "data/legal_texts.rds")
```
