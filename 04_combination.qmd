...
# 1. Noms uniques dans chaque base
legal_names <- legal_texts %>%
  distinct(ap_match_sapm) %>%
  rename(name_legal = ap_match_sapm)

sapm_names <- pa_names %>%
  distinct(short_name_sapm_2017) %>%
  rename(name_sapm = short_name_sapm_2017)

# 2. Vérifier les noms de `legal_texts` qui ne sont pas dans `pa_names`
not_in_sapm <- anti_join(legal_names, sapm_names, by = c("name_legal" = "name_sapm"))

# 3. Vérifier les noms de `pa_names` qui ne sont pas dans `legal_texts`
not_in_legal <- anti_join(sapm_names, legal_names, by = c("name_sapm" = "name_legal"))

# 4. Résumé clair
cat("Nombre d’identifiants légaux uniques :", nrow(legal_names), "\n")
cat("Nombre d’identifiants SAPM 2017 uniques :", nrow(sapm_names), "\n")
cat("Noms dans legal_texts absents de pa_names :", nrow(not_in_sapm), "\n")
cat("Noms dans pa_names absents de legal_texts :", nrow(not_in_legal), "\n")

# 5. Affichage facultatif des noms non appariés
not_in_sapm %>% arrange(name_legal)
not_in_legal %>% arrange(name_sapm)
```
