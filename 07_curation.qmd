...
## Example Amendment Files

Let's view an example of each type:

```{r}
#| label: show-example-amendments

# Show first spatial amendment
if (any(exported$kind == "spatial")) {
  spatial_example <- exported |> filter(kind == "spatial") |> slice(1)
  cat("\n=== Example Spatial Amendment ===\n")
  cat(readLines(spatial_example$yaml_path), sep = "\n")
}

# Show first attribute amendment
if (any(exported$kind == "attribute")) {
  attribute_example <- exported |> filter(kind == "attribute") |> slice(1)
  cat("\n\n=== Example Attribute Amendment ===\n")
  cat(readLines(attribute_example$yaml_path), sep = "\n")
}
```

## Next Steps

The exported YAML files in `data/amendments/` are ready to be:

1. **Reviewed**: Check YAML syntax and content correctness
2. **Committed**: Add to version control (git)
3. **Submitted**: Transfer to `dynamic_wdpa` repository
4. **Maintained**: Edit directly or regenerate from updated curation

The R working tables (`*_working.rds`) remain available for local analysis and can be used to regenerate exports if the schema or data changes.
