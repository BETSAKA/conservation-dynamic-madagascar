...
fat <- fat |>
  ...
  mutate(
    valid_to = if (cur_group()$amendment_type == "temporary_protection" && n() > 1) {
...
fat <- fat |>
  ...
  mutate(
    valid_to = case_when(
      amendment_type %in% c("correction", "status_change") &
...
cat("  SAT rows:", n_sat_before, "->", nrow(sat),
    "(", n_sat_before - nrow(sat), "duplicates merged)\n")
```
