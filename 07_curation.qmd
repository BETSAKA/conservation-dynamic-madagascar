...
Sur la base de ces éléments, nous retenons donc une période de protection temporaire à partir du `18 octobre 2008`, suivie de la création définitive du Paysage Harmonieux Protégé le `28 avril 2015`. 

On extrait des données de subdivision les zones correspondantes à l'AP en 2008.
...
tm_shape(loky_sub) +
  ...
  tm_layout(main.title = "Bloc Forestier Loky Manambato (2008)")
```

```{r loky-manambato-subdivision}
loky_current <- all_PAs_conso |>
  filter(WDPAID == 555697908, dataset_id == "WDPA_2025")

loky_noyau_dur <- st_read(
  ...
  mutate(
    WDPAID = 555697908,
    NAME = "Loky Manambato",
    dataset_id = "MEDD_subdivisions_2024",
    pa_label = "Manambato"
  )

tm_shape(loky_current) +
  tm_borders(col = "red", lwd = 2) +
  tm_shape(loky_noyau_dur) +
  tm_polygons(fill = "darkgreen", fill_alpha = 0.5, col = "darkgreen")
```

**Modifications FAT et SAT** : Nous documentons dans FAT une période de protection temporaire `2008-2015`, sans retenir `2005` comme date de création de l'aire protégée. Nous documentons dans SAT le noyau dur actuel comme zonage secondaire distinct de la limite principale de l'AP.

```{r loky-manambato-fat-sat}
# FAT: période de protection temporaire retenue à partir de l'arrêté 52005/2010
fat <- add_modif_fat(
  fat = fat,
  decisions = pa_decisions,
  wdpa_name_or_id = 555697908,
  manual_decision = list(
    date_texte = as.Date("2010-12-20"),
  ...
  valid_to = "2015-04-28",
  notes = "Temporary protection retained from Arrêté 52005/2010, cited by Arrêté 15862/2013 and Décret 2015-758. The 2005 SFUM arrêté is not treated here as a protected-area status."
)

# SAT: noyau dur actuel validé dans les données de subdivision
sat <- add_modif_sat(
  sat = sat,
  data = loky_noyau_dur,
  decisions = pa_decisions,
  ...
  manual_decision = list(
    ...
    num_texte = "MEDD-2024",
    id_texte = NA,
    objet_texte = "Noyau dur validé pour Loky Manambato"
  ),
  source_dataset_id = "MEDD_subdivisions_2024",
  amendment_type = "secondary_zoning",
  valid_from = NULL,
  notes = "Current core zone (noyau dur) from validated subdivision data for Loky Manambato, analyzed in 06_subdivisions.qmd. This zoning is documented separately from the legal PA boundary."
)
```
