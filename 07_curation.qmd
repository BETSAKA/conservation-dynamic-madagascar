...
- **`secondary_zoning`**: Additional zoning layers (core zones, buffer zones) that complement the main PA boundary
...
Le changement de statut de 1997 (RNI → PN) constitue un changement d'attributs à documenter dans FAT. Bien que ce décret ne soit pas référencé dans CNLEGIS, il a été retrouvé et archivé. Nous documentons deux périodes via FAT : la période RNI (1939-1997) avec `STATUS_YR=1939`, puis la période PN (1997-présent) avec `STATUS_YR=1997` conformément au manuel WDPA.
...
fat <- fat |>
  add_modif_fat(
    ...
    valid_to = "1997-08-07",
    notes = "Created as RNI #11 on 1939-06-11 (Décret du 11 juin 1939), boundaries clarified 1966-06-01 (Décret 66-242). Status changed to PN in 1997."
  ) |>
  add_modif_fat(
    decisions = pa_decisions,
    wdpa_name_or_id = "Andohahela",
    ...
    notes = "Changed from RNI to PN via Décret 97-1043 du 7 août 1997. Three-parcel structure maintained. Decree not in CNLEGIS but archived independently. Reference: https://archive.org/details/decret-n-97-1043-du-07-aout-1997-pn-andoahahela. WDPA 2025 shows STATUS_YR=1997 conformément au manuel WDPA."
  )
```
...
fat <- fat |>
  add_modif_fat(
    ...
    valid_to = "1998-08-19",
    notes = "Created as RNI #5 on 1927-12-31 (Décret du 31 décembre 1927), boundaries fixed 1966-06-01 (Décret 66-242). Status changed to PN in 1998."
  ) |>
  add_modif_fat(
    decisions = pa_decisions,
    wdpa_name_or_id = "Andringitra",
    ...
    amendment_type = "status_change",
    valid_from = "1998-08-19",
    notes = "Changed from RNI to PN via Décret 98-376 du 19 août 1998. Boundaries remained unchanged from 1966 delimitation. WDPA 2025 shows STATUS_YR=1998 conformément au manuel WDPA."
  )
```
...
fat <- fat |>
  ...
  add_modif_fat(
    ...
    amendment_type = "status_change",
    valid_from = "2002-08-07",
    notes = "Changed from RNI to PN via Décret 2002-798 du 7 août 2002. RNI + forest reserves + Station Forestière Ampijoroa consolidated with major boundary expansion. Reference: Alonso & Shuldenberg 2002. WDPA 2025 shows STATUS_YR=2002 conformément au manuel WDPA."
  )
```
...
fat <- fat |>
  add_modif_fat(
    ...
    valid_from = "1927-12-31",
    valid_to = "2011-09-06",
    notes = "Created as RNI #9 on 1927-12-31 (Décret du 31 décembre 1927), boundaries modified 1997-08-07 (Décret 97-1045). Status changed to PN in 2011."
  ) |>
  add_modif_fat(
    decisions = pa_decisions,
    wdpa_name_or_id = "Bemaraha",
    ...
    notes = "Changed from RNI to PN via Décret 2011-498 du 6 septembre 2011 with boundary modification. WDPA 2025 shows STATUS_YR=2011 conformément au manuel WDPA."
  )
```
...
fat <- fat |>
  add_modif_fat(
    ...
    valid_to = "1998-05-19",
    notes = "Created as RNI #12 on 1952-01-03 (Décret du 3 janvier 1952), boundaries clarified 1966-06-01 (Décret 66-242). Status changed to PN in 1998."
  ) |>
  add_modif_fat(
    decisions = pa_decisions,
    wdpa_name_or_id = "Marojejy",
    ...
    notes = "Changed from RNI to PN via Décret 98-375 du 19 mai 1998. No boundary change. WDPA 2025 shows STATUS_YR=1998 conformément au manuel WDPA."
  )
```
...
fat <- add_modif_fat(
  ...
  valid_from = "1958-10-28",
  valid_to = "2015-04-28",
  notes = "Created as PN on 1958-10-28 via Décret 58-07. Enlarged in 2015 through merger with RS Forêt d'Ambre (WDPAID 5025)."
)

# FAT for WDPAID 2314: Enlarged PN status (2015-present)
fat <- add_modif_fat(
  fat = fat,
  decisions = pa_decisions,
  wdpa_name_or_id = 2314,
  ...
  notes = "Enlarged via Décret 2015-776 through merger with RS Forêt d'Ambre (WDPAID 5025) and inclusion of Station Forestière des Roussettes."
)
...
# FAT for WDPAID 5025: Dissolved status (2015-present)
fat <- add_modif_fat(
  fat = fat,
  decisions = pa_decisions,
  wdpa_name_or_id = 5025,
  ...
  amendment_type = "status_change",
  valid_from = "2015-04-28",
  notes = "RS Forêt d'Ambre absorbed into WDPAID 2314 via Décret 2015-776. STATUS changed to 'Dissolved'. WDPAID 5025 remains in database for historical traceability."
)
```
...
fat <- add_modif_fat(
  ...
  valid_from = "1927-12-31",
  valid_to = "2002-08-07",
  notes = "Created as RNI #8 on 1927-12-31 (Décret du 31 décembre 1927). Status changed to PN in 2002."
)

# FAT for current status: PN period (2002-present)
fat <- add_modif_fat(
  fat = fat,
  decisions = pa_decisions,
  wdpa_name_or_id = "Namoroka",
  ...
  amendment_type = "status_change",
  valid_from = "2002-08-07",
  notes = "Upgraded from RNI to PN via Décret 2002-796 du 7 août 2002. No boundary change."
)
```
...
fat <- add_modif_fat(
  ...
  valid_from = "1965-12-14",
  valid_to = "2015-04-28",
  notes = "Created as RS (Réserve Spéciale) on 1965-12-14 via Décret 65-795. Status changed to PN in 2015."
)

# FAT for current status: PN period (2015-present)
fat <- add_modif_fat(
  fat = fat,
  decisions = pa_decisions,
  wdpa_name_or_id = "Nosy Mangabe",
  ...
  amendment_type = "status_change",
  valid_from = "2015-04-28",
  notes = "Upgraded from RS to PN via Décret 2015-775 du 28 avril 2015, with boundary expansion (added 4 small islets)."
)
```
...
fat <- add_modif_fat(
  ...
  valid_from = "1927-12-31",
  valid_to = "2002-08-07",
  notes = "Created as RNI #10 on 1927-12-31 (Décret du 31 décembre 1927). Status changed to PN in 2002."
)
...
fat <- add_modif_fat(
  ...
  amendment_type = "status_change",
  valid_from = "2002-08-07",
  notes = "Changed from RNI to PN via Décret 2002-797 du 7 août 2002. Boundary later expanded in 2015 to include Hatokaliotsy extension."
)
```
...
fat <- add_modif_fat(
  ...
  valid_from = "1997-08-07",
  valid_to = "2015-04-21",
  notes = "Décret 97-1044 established mixed status: PN (42,300 ha, IUCN II) + RNI (22,100 ha, IUCN Ia) with Antenina enclave (350 ha). Boundaries reduced from ~73,160 ha to 64,378 ha."
)

# FAT for unified status: PN only (2015-present)
fat <- add_modif_fat(
  fat = fat,
  decisions = pa_decisions,
  wdpa_name_or_id = "Zahamena",
  ...
  notes = "Unified to PN only via Décret 2015-737 du 21 avril 2015, eliminating previous composite PN+RNI zoning. Boundaries expanded from 64,378 ha to 64,935 ha."
)
```
...
Chaque noyau dur validé est ajouté à la table SAT avec le type d'amendement `secondary_zoning`. Ces zonages représentent l'état actuel documenté par les gestionnaires (transmission MEDD 2025) et n'ont pas de dates de validité temporelle spécifiques.

```{r add-secondary-zones-to-sat}
# Add each validated core zone to SAT as secondary_zoning amendment
for (i in seq_len(nrow(noyaux_durs))) {
  nd_row <- noyaux_durs[i, ]

  # Extract geometry as sf object for add_modif_sat
  nd_geom <- nd_row |>
    select(geometry) |>
    mutate(
      WDPAID = nd_row$WDPAID,
      NAME = nd_row$NAME,
  ...
  sat <- add_modif_sat(
    ...
    manual_decision = list(
      date_texte = as.Date("2024-01-01"),
      type_texte = "Zonage validé",
      num_texte = "MEDD-2024",
      objet_texte = paste(
    ...
    notes = paste0(
      ...
      nd_row$pa_label,
      ". ",
      "This represents current management zoning, not a temporal amendment. ",
      "17 PAs with exploitable core zones as of 2024."
    )
  )
}

cat("\nSAT entries created for", nrow(noyaux_durs), "core zones\n")
