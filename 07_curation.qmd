...
#   amendment_type: Type of spatial amendment (default: "boundary_modification")
...
# that differ from current WDPA representation. It uses a long format where
# each changed attribute becomes its own row, all linked to the same legal
# decision.
#
# Parameters:
...
#   amendment_type: Type of attribute amendment (e.g., "status_change",
#                   "governance_change", "designation_change")
#   valid_from: Optional start date (Date or character). If NULL, remains NA
#   confidence: Confidence level ("high", "medium", "low")
#   verification_method: Method used to verify the attributes
#   verified_by: Who performed verification
#   secondary_reference: Optional citation (e.g., "Goodman et al. 2018")
#   notes: Optional notes (defaults to decision object text)
#
# Returns:
#   Updated FAT table (tibble) with new amendment(s) appended. Each attribute
#   in the 'attributes' list becomes a separate row in long format.
...
add_modif_fat <- function(
  ...
  # Create one FAT entry per attribute (long format)
  fat_entries <- tibble(
    attribute_name = names(attributes),
    attribute_value = as.character(attributes)
  ) %>%
    mutate(
      WDPAID = wdpaid,
      valid_from = if (!is.null(valid_from)) {
      ...
      attribute_type = case_when(
        attribute_name %in% c("STATUS_YR", "REP_AREA", "GIS_AREA") ~ "numeric",
        attribute_name %in% c("STATUS_DATE") ~ "date",
        TRUE ~ "character"
      ),
      ...
      legal_instrument_source = "CNLEGIS",
      legal_instrument_id = dec$id_texte[1],
      legal_instrument_url = if ("cnlegis_url" %in% names(dec)) {
        dec$cnlegis_url[1]
      } else {
        NA_character_
      },
  ...
  if (is.null(fat)) {
    ...
    fat_new <- bind_rows(fat, fat_entries)
  }
