...
fat <- fat %>%
  add_modif_fat(
    ...
    notes = "RNI #7 created 1927, boundaries clarified 1966. RNI (+ forest reserves + Station Forestière Ampijoroa) consolidated into PN in 2002 (Décret 2002-798 du 7 août 2002). Major boundary expansion."
  ) %>%
  add_modif_fat(
    decisions = pa_decisions,
    wdpa_name_or_id = "Ankarafantsika",
...
On n'a pas de source géométrique antérieure aux années 2000 pour cette AP.
...
sat <- add_modif_sat(
  ...
  decision_filter = NULL, # No decision changes geometry for 5025
  source_dataset_id = "MNP_2010_split",
  amendment_type = "dissolved_pa_geometry",
  valid_from = "1958-10-28",
  notes = "Geometry of RS Forêt d'Ambre (polygons 2+3 from MNP_2010, 45.60 km²). Geometry remains constant, but PA received STATUS='Dissolved' in 2015 when absorbed into WDPAID 2314. Attributes from WDPA 2010."
)
```
...
fat <- add_modif_fat(
  ...
  attributes = list(
    ...
    merged_into_wdpaid = "2314",
    merger_date = "2015-04-28"
  ),
  amendment_type = "merger_dissolution",
  valid_from = "2015-04-28",
  notes = "RS Forêt d'Ambre (created 1958 by Décret 58-15) was absorbed into WDPAID 2314 via Décret 2015-776. STATUS changed from 'Designated' to 'Dissolved'. WDPAID 5025 remains in database for historical traceability but with dissolved status."
)
```
...
Cependant, WDPA 2025 présente plusieurs **erreurs majeures** pour le WDPAID 2306 :
...
add_modif_fat(
  ...
  decision_filter = list(num_texte = "2015-782"),
  attributes = list(
    DESIG = "Réserve Naturelle Intégrale",
    DESIG_ENG = "Strict Nature Reserve",
    IUCN_CAT = "Ia"
  ),
  amendment_type = "attribute_correction",
  valid_from = "1927-12-31",
  confidence = "high",
  verification_method = "legal_text_analysis",
  verified_by = "project_team",
...
fat <- add_modif_fat(
  ...
  attributes = list(
    ...
    IUCN_CAT = "II+Ia",
    composite_zones = "PN (42,300 ha, IUCN II) + RNI (22,100 ha, IUCN Ia)",
    enclave_antenina = "350 ha"
  ),
  amendment_type = "status_change",
  valid_from = "1997-08-07",
  notes = "Mixed PN+RNI status from 1997-2015 per Décret 97-1044. Unified to PN only in 2015 via Décret 2015-737."
)
...
## Identification des décisions légales sans modification spatiale



### Montagne des Français
Goodman	et	al.	[-@goodman2018]	indique	pour	cette	AP:
> Aspects	légaux	:	Gestionnaire	actuel	:	Service	d’Appui	à	la	Gestion	de
l’Environnement	(SAGE)	;	Gestion	antérieure	:	aucune	;	Création	:	selon	le
Décret	n°	2015-780	du	28	avril	2015	;	Modification	de	statut	:	une	partie	de
la	Réserve	Forestière	de	la	Montagne	des	Français,	mise	en	place	en	1955
selon	l’Arrêté	n°	823-DOM	du	2	avril	1955	(5571	ha),	est	intégrée	dans	le
paysage	harmonieux	protégé	créé	en	2015	suivant	le	Décret	n°	2015-780	du
28	avril	2015	;	Dernière	modification	de	statut	:	aucune	;	Surface	actuelle	:
6049	ha	(selon	le	décret	en	vigueur).
...
Statyut IV ou II ??? (dans WDPA 2025 c'est II)
