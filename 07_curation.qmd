...
::: {.callout-warning}
## Limitation des URLs CNLEGIS

Les liens vers CNLEGIS construits automatiquement a partir du numero de texte (par exemple `https://cnlegis.gov.mg/page_num_find_direct/?2015-730`) ne redirigent pas directement vers le decret concerne mais vers une page de recherche generique. Cette limitation empeche la verification directe des sources a partir des URLs. Les numeros de texte (`num_texte`) restent neanmoins fiables comme identifiants pour une recherche manuelle sur le site CNLEGIS.
:::
...
add_modif_sat <- function(
  ...
  if (amendment_type == "boundary_modification" && is.null(valid_from)) {
    warning(
      "add_modif_sat(): boundary_modification for WDPAID=",
      wdpaid,
...
temporal_description: "Modification des limites du 1958-06-08 au 2015-05-06 selon décret n. 2015-776 du 2015-05-06."
...
temporal_description: "Correction de donnees du 1956-02-20 au 2015-08-10 selon décret n. 56-208 du 1956-02-20."
...
Les données WDPA pour le `WDPAID 352239` documentent une protection temporaire en 2007 puis une création définitive en 2015. Le cas est particulier, car la décision CNLEGIS disponible pour 2007 est une prorogation qui cite explicitement un arrêté initial de 2006 absent de la base CNLEGIS.

```{r ambatotsirongorongo-cnlegis}
show_cnlegis_for_pa(352239, pa_decisions)
```

Le fragment pertinent de la décision CNLEGIS est le suivant :

> "Vu l'arrêté n° 3750 / 2006-MINENVEF du 15 mars 2006 portant protection temporaire de l'aire protégée en création dénommée \"Ambatotsirongorongo\","
>
> "En application de l'article 2 de l'arrêté n° 3750/2006 du 15 mars 2006 portant protection temporaire de l'Aire Protégée en création dénommée \"Ambatotsirongorongo\", la durée de la mise en protection temporaire de l'Aire Protégée en création \"Ambatotsirongorongo\" est prorogée de douze (12) mois."

Même si l'arrêté n° 3750/2006-MINENVEF du 15 mars 2006 n'est pas retrouvé directement dans CNLEGIS, l'arrêté de prorogation n° 13613/2007 du 20 août 2007 atteste clairement l'existence d'une première mise en protection temporaire en 2006. Nous retenons donc deux périodes temporaires successives dans l'historique juridique : une période initiale à partir du 15 mars 2006, puis une prorogation à partir du 20 août 2007, avant le décret de création définitive n° 2015-792 du 28 avril 2015. Comme il ne s'agit pas ici d'un amendement sur une protection temporaire, le traitement correspondant est centralisé dans la section @sec-temp-identification.
...
sat <- NULL |>
  add_modif_sat(
    ...
    notes = "Périmètre antérieur au décret modifiant (géométrie MNP_2010 retenue après vérification visuelle). Création par Décret 58-10 du 28 octobre 1958."
  )
```
...
sat <- sat |>
  add_modif_sat(
    ...
    notes = "Périmètre antérieur au décret modifiant (géométrie SAPM_2010 retenue après vérification visuelle). Création comme RNI #11 le 11 juin 1939."
  )
```
...
L'extension de 2002 a fait l'objet d'une cartographie détaillée par Conservation International [@alonso2002]. Le MNP a fourni le périmètre de la Réserve Naturelle Intégrale n. 7 telle que délimitée par le Décret 66-242 (~57 000 ha), antérieur a la consolidation de 2002.

![Ankarafantsika 2002 expansion](data/Alonso_Shuldenberg_2002.png){fig-alt="Carte d'Ankarafantsika avant 2002."}

```{r ankarafantsika-sources-pre2002}
plot_pa_sources(
  "Ankarafantsika",
  sources = c("MNP_Anka_RNI_pre2002", "ANGAP_2002", "WDPA_2025")
)
```

Les textes légaux identifient plusieurs décisions modificatives, notamment en 2002 (création du PN par consolidation) et 2015 (modification des limites). On documente les deux transitions :

1. **Périmètre RNI pré-2002** (MNP_Anka_RNI_pre2002) : la géométrie de la RNI n. 7 avant la consolidation en PN.
2. **Périmètre PN 2002-2015** (ANGAP_2002) : le PN après consolidation mais avant la modification de 2015.

```{r ankarafantsika-sat-pre2002}
# SAT: pre-2002 RNI boundary (Décret 66-242)
sat <- sat |>
  add_modif_sat(
    data = all_PAs_conso,
    decisions = pa_decisions,
    ...
    source_dataset_id = "MNP_Anka_RNI_pre2002",
    valid_from = "1927-12-31",
    notes = "Périmètre de la RNI n. 7 d'Ankarafantsika (Décret 66-242, ~57 000 ha) avant la consolidation en PN en 2002 (Décret 2002-798, 130 000 ha). Géométrie fournie par MNP."
  )
```

```{r ankarafantsika-sat}
# SAT: PN boundary 2002-2015
sat <- sat |>
  add_modif_sat(
    data = all_PAs_conso,
    decisions = pa_decisions,
    ...
    notes = "Périmètre 2002-2015 avant modification des limites (géométrie ANGAP_2002 retenue après vérification visuelle)."
  )
```
...
```{r befotaka-fat}
# FAT: Correction du STATUS_YR
# Using manual_decision because Befotaka-Midongy (WDPAID 20272) is not matched
# in the CNLEGIS extraction pipeline (chapter 03 name-matching gap).
fat <- add_modif_fat(
  fat = fat,
  decisions = pa_decisions,
  wdpa_name_or_id = 20272,
  manual_decision = list(
    date_texte = "1997-12-18",
    type_texte = "Decret",
    num_texte = "97-1451"
  ),
  attributes = list(
  ...
  notes = "WDPA incorrectly shows STATUS_YR=1953 (date of classified forests that were not protected areas). PN Befotaka-Midongy du Sud was created in 1997 (Decret 97-1451), incorporating parts of the Soarano and Befotaka classified forests from 1953."
)
```
...
Les données WDPA pour le `WDPAID 555697908` indiquent un `STATUS_YR=2015`, correspondant au décret de création définitive `2015-758`. Les textes CNLEGIS actuellement appariés documentent directement ce décret ainsi qu'un arrêté de délégation de gestion temporaire `15862/2013`.

```{r loky-manambato-cnlegis}
show_cnlegis_for_pa(555697908, pa_decisions)
```

Des sources complémentaires suggèrent une antériorité plus ancienne. Le PGES finalisé en 2010 indique que le territoire de Loky-Manambato bénéficiait auparavant d'une protection temporaire puis d'un statut de Station Forestière à Usage Multiple (SFUM) par l'arrêté `5862/05-MINENVEF` du `31 mai 2005`. En cohérence avec la logique retenue dans ce travail, ce statut de SFUM n'est pas considéré ici comme un statut d'aire protégée en lui-même. Nous ne retenons donc pas `2005` comme début de conservation.

L'arrêté `15862/2013` précise qu'il est pris en application de l'article 4 de l'arrêté `52005/2010`, et le décret `2015-758` cite également la chaîne `18633/2008`, `52005/2010` et `9874/2013`. Après analyse visuelle, nous identifions sur la carte annexée à l'arrêté `18633/2008` quatre zones de protection temporaire correspondant à Loky Manambato :

[Extrait de la carte annexée au décret de protection globale temporaire 18633/2008](sources/map_temp_prot_2008_loky_manambato.png)

Elles correspondent aux 4 parties de la station forestière créée en 2005 et représentées dans le PGES de 2010 :

[Carte incluse dans le PGES de 2010 pour Loky Manambato](sources/pges_map_2010_loky_manambato.png)

Sur la base de ces éléments, nous retenons donc une période de protection temporaire à partir du `17 octobre 2008`, suivie de la création définitive du Paysage Harmonieux Protégé le `28 avril 2015`.

On extrait des données de subdivision la géométrie correspondant au périmètre antérieur à 2015. Le fichier `Bloc Forestier Loky Manambato.shp` contient plusieurs polygones, qui sont fusionnés en une seule géométrie avant intégration dans SAT, conformément aux autres cas analogues du chapitre.

```{r loky-manambato-geom-prep}
loky_current <- all_PAs_conso |>
  filter(WDPAID == 555697908, dataset_id == "WDPA_2025")

loky_sub <- read_sf(
  "data/subdivisions/Manambato/Bloc Forestier Loky Manambato.shp"
) |>
  st_make_valid() |>
  st_transform(st_crs(loky_current)) |>
  st_union() |>
  st_sf() |>
  ...
  mutate(
    WDPAID = 555697908,
    NAME = "Loky Manambato",
    dataset_id = "MEDD_subdivisions_2008"
  )

tm_shape(loky_sub) +
  tm_polygons(fill = "darkgreen", fill_alpha = 0.5, col = "darkgreen") +
  tm_title("Bloc Forestier Loky Manambato (pré-2015)")
```

```{r loky-manambato-sources}
tm_shape(loky_current) +
  tm_borders(col = "red", lwd = 2) +
  tm_shape(loky_sub) +
  tm_polygons(fill = "darkgreen", fill_alpha = 0.5, col = "darkgreen")
```

**Modifications FAT et SAT** : Nous documentons dans FAT une période de protection temporaire `2008-2015`, sans retenir `2005` comme date de création de l'aire protégée. Nous documentons dans SAT la géométrie pré-2015 obtenue à partir du bloc forestier, les polygones ayant été fusionnés en une seule géométrie. Le noyau dur actuel reste quant à lui intégré dans le traitement générique du zonage secondaire plus bas dans le chapitre.

```{r loky-manambato-fat-sat}
# FAT: période de protection temporaire retenue à partir de l'arrêté 18633/2008
fat <- add_modif_fat(
  fat = fat,
  decisions = pa_decisions,
  wdpa_name_or_id = 555697908,
  manual_decision = list(
    date_texte = as.Date("2008-10-17"),
    type_texte = "Arrêté interministériel",
    num_texte = "18633/2008",
    id_texte = NA,
    objet_texte = "Mise en protection temporaire globale des sites visés par l'arrêté n°17914 du 18 octobre 2006, incluant Loky Manambato"
  ),
  attributes = list(
    STATUS = "Designated",
    STATUS_YR = "2008",
  ...
  valid_to = "2015-04-28",
  notes = "Temporary protection retained from Arrêté 18633/2008, supported by the annex map, by the PGES 2010 map, and by later references in Arrêté 15862/2013 and Décret 2015-758. The 2005 SFUM arrêté is not treated here as a protected-area status."
)

# SAT: géométrie pré-2015 issue du bloc forestier, fusionnée en une seule entité
sat <- add_modif_sat(
  sat = sat,
  data = loky_sub,
  decisions = pa_decisions,
  wdpa_name_or_id = 555697908,
  decision_filter = list(num_texte = "2015-758"),
  source_dataset_id = "MEDD_subdivisions_2008",
  amendment_type = "boundary_modification",
  valid_from = "2008-10-17",
  notes = "Pre-2015 boundary from the Loky Manambato forest block subdivision layer. The original shapefile contains four polygons; they are merged with st_union() into a single geometry before insertion in SAT. This geometry matches the temporary protection footprint visible on the annex map to Arrêté 18633/2008 and on the PGES 2010 map."
)
```
...
Le MNP a fourni le périmètre original de 1989 (Décret 89-011, ~9 930 ha), antérieur a l'extension de 2002 (Décret 2002-790, 15 480 ha). Ce périmètre est disponible dans le jeu `MNP_Mantadia_pre2002`.

```{r mantadia-sources}
plot_pa_sources(
  "Mantadia",
  sources = c("MNP_Mantadia_pre2002", "WDPA_2025")
)
```

Conformément aux regles de construction de la SAT, on ajoute la géométrie antérieure a la modification de 2002.
...
sat <- sat |>
  add_modif_sat(
    data = all_PAs_conso,
    decisions = pa_decisions,
    wdpa_name_or_id = "Mantadia",
    decision_filter = list(year = 2002, type_decision_pattern = "modifiant"),
    source_dataset_id = "MNP_Mantadia_pre2002",
    valid_from = "1989-01-11",
    notes = "Périmètre original du PN Mantadia (Décret 89-011, ~9 930 ha) avant l'extension de 2002 (Décret 2002-790, 15 480 ha). Géométrie fournie par MNP."
  )
```
...
> Aspects légaux : Aspect	légaux	:	Gestionnaire	actuel	:	Madagascar	National	Parks	(MNP)	; Gestion	antérieure	:	aucune	;	Création	:	selon	le	Décret	n°	56-208	du	20 février	1956	(Création	de	la	Réserve	Spéciale)	;	Modification	de	statut	: aucune	;	Dernière	modification	de	statut	:	aucune	;	Surface	actuelle	:	42 200	ha	(selon	le	décret	en	vigueur).

Dans la WDPA 2025, Marotandrano apparaît avec `STATUS_YR=2015`. Or cette AP est toujours une Réserve Spéciale de catégorie IV, et Goodman et al. 2018 n'indiquent aucun changement de statut. 
...
fat <- fat |>
  add_modif_fat(
    decisions = pa_decisions,
    wdpa_name_or_id = 5035,
    manual_decision = list(
      date_texte = as.Date("1956-02-20"),
      type_texte = "Décret",
      num_texte = "56-208",
      objet_texte = "Création de la Réserve Spéciale de Marotandrano"
    ),
    attributes = list(
      STATUS_YR = "1956"
    ),
    amendment_type = "correction",
    valid_from = "1956-02-20",
    notes = "WDPA incorrectly shows STATUS_YR=2015. Goodman et al. 2018 report Marotandrano as a Special Reserve created in 1956 (Décret 56-208) with no subsequent status change."
  )
```
...
Les données WDPA pour le `WDPAID 555697902` documentent des protections temporaires successives en 2008, 2010 et 2013, puis une création définitive en 2015. Or le décret de création définitive `2015-777` cite explicitement un arrêté initial de protection temporaire en 2006, absent de la base structurée `pa_decisions`.

```{r petriky-cnlegis}
show_cnlegis_for_pa(555697902, pa_decisions)
```

Le fragment pertinent du décret CNLEGIS est le suivant :

> "Vu l'arrêté Interministériel n°7880/2006-MINENVEF du 15 mai 2006 portant protection temporaire de l'Aire Protégée en création Forêt de Petriky ;"

Nous retenons donc une première période de protection temporaire à partir du `15 mai 2006`, puis les actes de 2008, 2010 et 2013 jusqu'au décret de création définitive `2015-777` du `28 avril 2015`. Comme pour les autres cas analogues, le traitement correspondant est centralisé dans la section @sec-temp-identification.
...
fat <- add_modif_fat(
  ...
  amendment_type = "correction",
  valid_from = "1927-12-31",
  notes = "WDPA 2025 incorrectly classifies Tsaratanàna as PN/II and shows STATUS_YR=1997. Decree 2015-782 confirms it remains RNI/Ia (Strict Nature Reserve). The RNI has never changed status since creation in 1927, so STATUS_YR should remain 1927. The 2015 decree created the CAPAM complex and added 59,279 ha extensions (also Ia), bringing total to 108,610 ha."
)
```
...
```{r tampoketsa-cnlegis}
show_cnlegis_for_pa(5036, pa_decisions)
```

WDPA 2025 montre `STATUS_YR=1959` pour Tampoketsa Analamaitso. Cependant, CNLEGIS associe cette aire protégée au Décret 58-14 du 28 octobre 1958, intitulé "Instituant la réserve spéciale de la forêt Tampoketsa d'Analamaitso". Nous retenons donc 1958 comme année de création à documenter dans FAT.
...
fat <- add_modif_fat(
  fat = fat,
  decisions = pa_decisions,
  wdpa_name_or_id = 5036,
  decision_filter = list(num_texte = "58-14"),
  attributes = list(
    STATUS_YR = 1958
  ),
  amendment_type = "correction",
  valid_from = "1958-10-28",
  notes = "WDPA incorrectly shows STATUS_YR=1959. CNLEGIS links Tampoketsa Analamaitso to Décret 58-14 du 28 octobre 1958 instituting the special reserve, so the creation year retained here is 1958."
)
```

### Tampolo

Les données CNLEGIS pour le `WDPAID 354011` documentent une série de protections temporaires entre 2007 et 2013, puis une création définitive en 2015. Comme pour Ambatotsirongorongo, la première décision CNLEGIS actuellement appariée est une prorogation qui cite explicitement un arrêté initial de 2006 absent de la base structurée.

```{r tampolo-cnlegis}
show_cnlegis_for_pa(354011, pa_decisions)
```

Le fragment pertinent de la décision CNLEGIS est le suivant :

> "Prorogeant l'arrêté n° 11475/2006-MINENVEF du 4 juillet 2006 portant protection temporaire de l'Aire protégée en création dénommée \"Tampolo\", District de Fénérive-Est, Région d'Analanjirofo."
>
> "En application de l'article 2 de l'arrêté n°11475/ 2006 du 04 juillet 2006 portant protection temporaire de l'Aire Protégée en création dénommée \"Tampolo\", la durée de la mise en protection temporaire de l'Aire Protégée en création \"Tampolo\" est prorogée de douze (12) mois."

Même si l'arrêté n° 11475/2006-MINENVEF du 4 juillet 2006 n'est pas retrouvé directement dans la base structurée CNLEGIS, l'arrêté de prorogation n° 13612/2007 du 20 août 2007 en atteste explicitement l'existence et maintient ses dispositions applicables. Nous retenons donc une première période de protection temporaire à partir du 4 juillet 2006, puis les actes de prorogation ou modification ultérieurs jusqu'au décret de création définitive n° 2015-789 du 28 avril 2015. Comme il s'agit ici d'un amendement sur une protection temporaire, le traitement correspondant est centralisé dans la section @sec-temp-identification.
...
fat <- add_modif_fat(
  ...
  valid_from = "1997-08-07",
  valid_to = "2015-04-28",
  notes = "Décret 97-1044 established mixed status: PN (42,300 ha, IUCN II) + RNI (22,100 ha, IUCN Ia) with Antenina enclave (350 ha). Boundaries reduced from ~73,160 ha to 64,378 ha. Unified to PN only via Décret 2015-737 (CNLEGIS date 2015-04-28), eliminating previous composite PN+RNI zoning. Boundaries expanded from 64,378 ha to 64,935 ha."
)
```
...
> Aspects légaux : Gestionnaire actuel : Madagascar National Parks (MNP) ; Gestion antérieure : Direction des Eaux et Forêts (Service de la Protection de la Nature d'Antananarivo et le Cantonnement Forestier de Sakaraha) ; Création : selon le Décret n° 97-1454 du 18 décembre 1997 ; Modification de statut : la Forêt Classée de Zombitse, localisée dans le Fivondronana de Sakaraha et gérée par le Cantonnement Forestier de Sakaraha à l'époque, existait déjà depuis 1962 (Arrêté n° 1902-MAP/FOR du 31 août 1962, 21 000 ha). Considéré comme Site d'Intérêt Biologique, cette forêt fait partie intégrante du parc national créé en 1998, selon le Décret n° 97-1454 du 18 décembre 1998 ; Dernière modification de statut : aucune ; Surface actuelle : 36 803 ha (selon le décret en vigueur).

```{r zombitse-cnlegis}
show_cnlegis_for_pa(20273, pa_decisions)
```

Le Décret 97-1454 du 18 décembre 1997 portant création du Parc National de Zombitse-Vohibasia apparaît bien dans la base CNLEGIS. WDPA 2025 montre pourtant `STATUS_YR=2015`, ce qui est incorrect. Cette date ne correspond pas au décret de création du parc national. La divergence provient vraisemblablement d'un décalage de nommage entre `Zombitse Vohibasia` dans la WDPA et `Zombitse-Vohibasia` dans les sources SAPM et juridiques.
...
fat <- add_modif_fat(
  fat = fat,
  decisions = pa_decisions,
  wdpa_name_or_id = 20273,
  decision_filter = list(num_texte = "97-1454"),
  attributes = list(
    STATUS_YR = 1997
  ),
  amendment_type = "correction",
  valid_from = "1997-12-18",
  notes = "WDPA incorrectly shows STATUS_YR=2015. CNLEGIS links Zombitse Vohibasia to Décret 97-1454 du 18 décembre 1997 creating the national park. The mismatch is reinforced by naming differences between 'Zombitse Vohibasia' (WDPA) and 'Zombitse-Vohibasia' (SAPM/legal sources)."
)
```
...
### Identification et traitement {#sec-temp-identification}
...
# Supplement with permanent decrees missing from CNLEGIS or mismatched.
# - Bongolava (352244): Decret 2015-790 absent de CNLEGIS (Goodman et al.)
# - Manjakatompo (555697911): Decret 2015-711, erreur de metadonnee CNLEGIS
#     qui l'attribue a Marolambo ; le texte integral confirme Manjakatompo
# - Tampolo (354011): Decret 2015-789 absent de CNLEGIS
# - Tsitongambarika (555697878): Decret 2015-720 absent de CNLEGIS
permanent_designations <- permanent_designations |>
  bind_rows(tibble::tribble(
    ~WDPAID   , ~perm_date            , ~perm_texte ,
       352244 , as.Date("2015-04-28") , "2015-790"  ,
    555697911 , as.Date("2015-04-21") , "2015-711"  ,
       354011 , as.Date("2015-04-28") , "2015-789"  ,
    555697878 , as.Date("2015-04-21") , "2015-720"
  ))

# Supplement with temporary decrees absent from CNLEGIS but explicitly cited
# in a later prorogation decision.
temporary_designations_missing <- tibble::tribble(
  ~WDPAID   , ~WDPA_NAME                 , ~temp_date            , ~temp_texte           , ~type_texte ,
     352239 , "Ambatotsirongorongo"      , as.Date("2006-03-15") , "3750/2006-MINENVEF"  , "Arrêté"    ,
     354011 , "Réserve de Tampolo"       , as.Date("2006-07-04") , "11475/2006-MINENVEF" , "Arrêté"    ,
  555697902 , "Forêt Naturel de Petriky" , as.Date("2006-05-15") , "7880/2006-MINENVEF"  , "Arrêté"
)
...
temp_full <- temporary_protections |>
  ...
  bind_rows(temporary_designations_missing) |>
  left_join(
...
# Exclude entries without a matched WDPAID (e.g. Efatsy Manombo, Arivomamo II,
# Miarinarivo II) that could not be linked to a WDPA record.
...
## Post-traitement : resolution des chevauchements temporels {#sec-post-processing}

Avant l'export, nous resolvons les chevauchements temporels dans la table FAT qui causeraient des conflits lors de la consolidation.

Certaines aires protegees ont recu plusieurs arretes de protection temporaire successifs (2008, 2010, 2013). Par defaut, chaque entree couvre la periode `[date_arrete, date_decret_definitif]`, ce qui cree des periodes chevauchantes avec des valeurs STATUS_YR differentes. Nous chainons ces entrees pour qu'elles deviennent sequentielles : chaque protection temporaire se termine quand la suivante commence.

De plus, pour les aires protegees qui disposaient deja d'un statut permanent avant de recevoir des protections temporaires (extensions de limites), nous ajustons les dates pour eviter les conflits entre les amendements de type correction/status_change et les protections temporaires.

```{r}
#| label: post-process-temporal-overlaps

n_before <- nrow(fat)

# 1. Chain overlapping temporary_protections: for each WDPAID with multiple
#    temp protections, sort by valid_from and set each valid_to = next valid_from.
#    The last entry keeps its original valid_to (permanent decree date or NA).
fat <- fat |>
  ...
  mutate(
    valid_to = if (
      cur_group()$amendment_type == "temporary_protection" && n() > 1
    ) {
      c(lead(valid_from)[-n()], valid_to[n()])
    } else {
      valid_to
    }
  ) |>
...
# 2. Fix temporal inversions: valid_to before valid_from is nonsensical.
#    This happens when a permanent decree predates the temporary protection
#    (old PAs that received new temp protections for boundary extensions).
n_inversions <- sum(
  !is.na(fat$valid_to) & !is.na(fat$valid_from) & fat$valid_to < fat$valid_from
)
fat <- fat |>
  mutate(
    valid_to = if_else(
      !is.na(valid_to) & !is.na(valid_from) & valid_to < valid_from,
      as.Date(NA),
      valid_to
    )
  )

# 3. For corrections/status_changes that overlap with later temp_protections,
#    end the correction when the first temp protection starts.
first_temp <- fat |>
  filter(amendment_type == "temporary_protection") |>
  group_by(WDPAID) |>
  summarise(first_temp_from = min(valid_from, na.rm = TRUE), .groups = "drop")

fat <- fat |>
  left_join(first_temp, by = "WDPAID") |>
  mutate(
    valid_to = case_when(
      amendment_type %in%
        c("correction", "status_change") &
        !is.na(first_temp_from) &
        (is.na(valid_to) | valid_to > first_temp_from) &
        !is.na(valid_from) &
        valid_from < first_temp_from ~ first_temp_from,
      TRUE ~ valid_to
    )
  ) |>
  select(-first_temp_from)

# 4. For temp_protections with no end date that overlap with a later
#    correction/status_change (permanent designation), end the temp protection
#    when the permanent designation starts.
later_perm <- fat |>
  filter(amendment_type %in% c("correction", "status_change")) |>
  group_by(WDPAID) |>
  summarise(latest_perm_from = max(valid_from, na.rm = TRUE), .groups = "drop")

fat <- fat |>
  left_join(later_perm, by = "WDPAID") |>
  mutate(
    valid_to = case_when(
      amendment_type == "temporary_protection" &
        is.na(valid_to) &
        !is.na(latest_perm_from) &
        !is.na(valid_from) &
        valid_from < latest_perm_from ~ latest_perm_from,
      TRUE ~ valid_to
    )
  ) |>
  select(-latest_perm_from)

# 5. Deduplicate SAT boundary_modifications with identical dates
#    (merge geometries into multipolygons).
n_sat_before <- nrow(sat)
sat <- sat |>
  group_by(WDPAID, amendment_type, valid_from, valid_to) |>
  summarise(
    across(where(~ !inherits(., "sfc")), first),
...
cat("  FAT rows:", n_before, "(unchanged)\n")
cat("  Temporal inversions fixed:", n_inversions, "\n")
cat(
  "  SAT rows:",
  n_sat_before,
  "->",
...
Le processus de curation a documente les amendements aux donnees WDPA de Madagascar. Nous exportons maintenant ces amendements vers le schema YAML standardise pour le registre `dynamic_wdpa`.
