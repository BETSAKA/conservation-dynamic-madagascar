...
wdpa_mdg_test <- wdpa_clean(wdpa_mdg)
...
Comme les périmètres se recoupent, on ajoute un remplissage pour le noyau dur. On voit que le noyau dur occupe une portion plus restreinte de l'AP comparé à d'autres.
On observe un décallage de la limite extérieure de l'AP entre les données transmises au MEDD et les données WDPA. Cela ne semble pas être un problème de projection.
...
En ce qui concerne les frontières de la NAP, les données fournies en .txt semblent concordantes avec celles de WDPA, hormis 2 points manquants et deux points comportant visiblement une erreur de saisie. 
Les données du noyau dur sont disponibles et sans anomalie apparente.
...
sub_bemanevika <- st_read(
  ...
  quiet = TRUE,
  options = "ENCODING=Windows-1252" # Specify encoding at read time
) |>
  st_make_valid() |>
  ...
  mutate(
    ...
    # Then create clean column
    SubZone_clean = case_when(
      str_detect(SubZone, "Noyau") ~ "Noyau Dur",
      str_detect(SubZone, "utilisation") ~ "Utilisation durable",
      str_detect(SubZone, "occupation") ~ "Occupation controlee",
      TRUE ~ as.character(SubZone)
    )
  ) |>
  select(-SubZone) # Drop the problematic original column

# Use clean names for mapping
vals <- c(
  "Noyau Dur" = "#1B9E77",
  "Utilisation durable" = "#D95F02",
  "Occupation controlee" = "#7570B3"
...
tm_shape(wdpa_bemanevika) +
  ...
  tm_polygons(
    fill = "SubZone_clean",
    fill.scale = tm_scale(values = vals),
    fill_alpha = 0.6,
    fill.legend = tm_legend(title = "Sous-zone")
  ) +
  tm_borders(col = "black", lwd = 1)

```

Le périmètre externe est proche de celui de WDPA et les périmètres internes semblent cohérents.
...
sub_sahafina_nd_shp <- st_read(
  "data/subdivisions/Sahafina/noyaudurpolygoneSahafina.shp",
  quiet = TRUE
) |>
  st_set_crs(4326) |>
  st_make_valid() |>
...
# En synthèse :
Alaotra : les données transmises contiennent uniquement le périmètre externe de l'AP.