...
for (yaml_path in yaml_files_sorted) {
  ...
  # Afficher le titre
  cat("\n### ", amendment_id, " - ", wdpa_name, " (WDPAID: ", wdpaid, ")\n\n", sep = "")
  ...
  if (!is.null(a$geometry_ref)) {
    ...
    if (file.exists(geom_path)) {
      ...
      cat("**Géométrie associée :** `", a$geometry_ref, "` (", 
          nrow(geom), " feature(s), ",
...
overlaps <- all_amendments |>
  ...
  mutate(
    ...
    valid_to = if_else(is.na(valid_to), as.Date("2100-01-01"), as.Date(valid_to))
  ) |>
...
if (nrow(overlaps) > 0) {
  cat("ℹ️ **Aires avec plusieurs amendements du même type (vérifier chevauchements)**\n\n")
  print(kable(overlaps |> select(amendment_id, wdpaid, amendment_type, valid_from, valid_to)))
} else {
