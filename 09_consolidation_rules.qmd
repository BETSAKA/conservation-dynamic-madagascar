...
## Téléchargement des sorties publiées

Les sorties dérivées de la WDPA produites par le CAR sont maintenant incluses dans ce dépôt et diffusées avec l'autorisation de l'UNEP-WCMC.

Attention : ces sorties dérivées de la WDPA, correspondent a un instantané historique limité a Madagascar, fondé sur la version de novembre 2025 de la WDPA. Elles peuvent donc devenir obsolètes. Pour accéder aux données officielles les plus récentes sur les aires protégées et conservées, consulter [Protected Planet](https://www.protectedplanet.net). Il est également possible, et recommandé, de reproduire une version à jour de ce jeu de données en téléchargeant la dernière version de la WDPA puis en réexecutant le protocole de consolidation documenté dans ce dépôt.
...
- [Télécharger le GeoPackage](https://github.com/BETSAKA/conservation-dynamic-madagascar/blob/main/data/dynamic_wdpa.gpkg?raw=1)
- [Télécharger le GeoParquet](https://github.com/BETSAKA/conservation-dynamic-madagascar/blob/main/data/dynamic_wdpa.parquet?raw=1)
- [Télécharger l'objet RDS](https://github.com/BETSAKA/conservation-dynamic-madagascar/blob/main/data/dynamic_wdpa.rds?raw=1)
- [Consulter le registre YAML des amendements](https://github.com/BETSAKA/conservation-dynamic-madagascar/tree/main/data/amendments)

Les données sources brutes (WDPA, SAPM, CNLEGIS) restent documentées dans ce dépôt mais continuent d'etre obtenues depuis leurs canaux d'origine.
...
generate_timeline <- function(wdpaid_val, wdpa, amendments) {
  ...
  # STATUS_YR fallback logic:
  # 1. Always used when no amendments exist (baseline-only PA).
  # 2. Also used when a boundary_modification has valid_to but no valid_from
  #    AND no other amendment provides a date earlier than that valid_to.
  #    This ensures the pre-modification period is represented.
  # 3. Skipped otherwise: amendments already provide the correct temporal
  #    segmentation and STATUS_YR would create spurious intervals.
  wdpa_date <- NULL
  if (nrow(wdpa_record) > 0) {
    status_yr_date <- as.Date(paste0(wdpa_record$STATUS_YR[1], "-01-01"))

    if (length(amendment_dates) == 0) {
      # Case 1: no amendments at all
      wdpa_date <- status_yr_date
    } else {
      # Case 2: check for spatial amendments missing valid_from
      spatial_no_from <- pa_amendments |>
        filter(amendment_kind == "spatial",
               amendment_type == "boundary_modification",
               is.na(valid_from),
               !is.na(valid_to))

      if (nrow(spatial_no_from) > 0) {
        earliest_valid_to <- min(spatial_no_from$valid_to)
        # Only add STATUS_YR if it is strictly earlier than all existing dates
        if (status_yr_date < min(amendment_dates)) {
          wdpa_date <- status_yr_date
        }
      }
    }
...
consolidate_pa_states <- function(wdpaid, wdpa, amendments) {
  ...
  states <- map_dfr(seq_len(nrow(timeline)), function(i) {
    ...
    #    then STATUS_YR = year(valid_from) of that amendment (= when this
    #      historical status started).
    # 2. No status amendment active in this interval:
    #    then STATUS_YR comes from WDPA baseline or from a `correction` amendment
