...
Les données spatiales sur les aires protégées sont essentielles pour les études de conservation. Cependant, les bases existantes se limitent souvent aux frontières actuelles, ignorant les évolutions historiques. Cette lacune engendre des biais dans l'analyse de leur impact environnemental et social. Dans ce projet, nous proposons un modèle de données dynamique documentant l'évolution des aires protégées à Madagascar de 2000 à 2024. Ce modèle inclut chaque état successif avec ses périodes de validité, fournissant une base analytique plus robuste. Les données seront publiées en accès libre pour favoriser leur utilisation par la communauté scientifique et les opérationnels.
...
2. **Segmentation temporelle** : Lorsque des décisions juridiques antérieures, des périmètres historiques ou des dispositifs de zonage secondaires sont identifiés, la chronologie est segmentée en plusieurs intervalles de validité qui ne se chevauchent pas.

3. **Héritage et compléments** : Les états antérieurs héritent par défaut des attributs WDPA, sauf lorsque des textes réglementaires, des délimitations datées ou des documents de zonage spécifient explicitement des différences (statut de protection temporaire, limites alternatives, changements de gouvernance, organisation interne distincte).
...
Les modifications apportées à WDPDA sont stockées sous forme de fichiers YAML. Nous avons choisi cette forme car c'est celle qui semble la plus facile à lire et amender manuellement. Ce format peut en outre être  versionnén afin que chaque modification soit tracée (dates, sources légales, types d'amendements). Les géométries associées sont stockées en GeoJSON. Ce registre constitue la référence centrale pour toutes les corrections et informations temporelles qui sont ensuite transposées.

Types d'amendements documentés :
- `boundary_modification` : Changements légaux des limites externes
- `status_change` : Modifications de désignation (RNI vers PN, RS vers PN, etc.)
- `correction` : Erreurs dans WDPA (dates, statuts, géométries)
- `secondary_zoning` : Zonages secondaires (zones noyaux, zones tampons)

**2. CAR (Consolidation and Auditing Rules)**  
Ensemble transparent de règles de consolidation, exprimées sous forme de code R reproductible, qui lit le registre YAML et la WDPA, applique les règles de précédence, et génère les états temporels. Le CAR garantit que la transformation WDPA vers états dynamiques est auditable, déterministe et reproductible.
...
Ces tables sont ensuite exportées vers le registre YAML, qui devient la référence unique. SAT/FAT ne sont donc que des outils de travail temporaires.
...
- **GeoParquet** : Format orienté colonnes optimisé pour analyses big data
...
**Partie 2 - Consolidation et validation** : Méthodologie d'appariement spatial et textuel des sources multiples, développement d'un outil de validation visuelle interactive (Shiny), et traitement systématique des zonages secondaires documentés par les gestionnaires.
...
Les données sources (SAPM, WDPA, CNLEGIS) restent externes au dépôt, mais les principales sorties dérivées et le registre d'amendements sont maintenant versionnés ici. Cette redistribution des sorties dérivées de la WDPA est effectuée avec l'autorisation de l'UNEP-WCMC.

Attention : les fichiers dérivés de la WDPA diffusés dans ce dépôt correspondent a un instantané historique limité a Madagascar, fondé sur la version de novembre 2025 de la WDPA. Ils peuvent donc devenir obsolètes. Pour accéder aux données officielles les plus récentes sur les aires protégées et conservées, consulter [Protected Planet](https://www.protectedplanet.net). Il est également possible, et recommandé, de reproduire une version à jour de ce jeu de données en téléchargeant la dernière version de la WDPA puis en réexecutant le protocole de consolidation documenté dans ce dépôt.

Source de base a citer pour les sorties dérivées de la WDPA : **UNEP-WCMC and IUCN (2025). Protected Planet: The World Database on Protected Areas (WDPA), version de novembre 2025. Cambridge, UK et Gland, Suisse. Disponible sur [Protected Planet](https://www.protectedplanet.net).**

Téléchargements directs :

- [GeoPackage dynamique](https://github.com/BETSAKA/conservation-dynamic-madagascar/blob/main/data/dynamic_wdpa.gpkg?raw=1)
- [GeoParquet dynamique](https://github.com/BETSAKA/conservation-dynamic-madagascar/blob/main/data/dynamic_wdpa.parquet?raw=1)
- [Objet R natif (.rds)](https://github.com/BETSAKA/conservation-dynamic-madagascar/blob/main/data/dynamic_wdpa.rds?raw=1)
- [Registre des amendements YAML](https://github.com/BETSAKA/conservation-dynamic-madagascar/tree/main/data/amendments)

Le code R est entièrement documenté et exécutable. 
...
- **Documenter les changements de statut** : Traçabilité des reclassements (p. ex. RS vers PN) ou abandons
