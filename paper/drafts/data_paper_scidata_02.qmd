...
The dataset was constructed through a multi-step pipeline integrating four complementary data sources, cross-referencing them through spatial and textual matching, curating amendments through expert review, and consolidating them into temporal states using deterministic rules. The complete methodology is documented in a reproducible notebook accompanying this paper (see supplementary material). We summarize the key steps below and illustrate the overall workflow in @fig-workflow.
...
- MNP 2010: Madagascar is the new name of ANGAP, at a time where new management agencies emerged (international or local NGOs)
...
- SAPM 2017 and 2024 : Updated datasets from the MEDD, who took over the coordination of the protected area system in 2015
...
```{r}
#| label: fig-examples
#| eval: !expr file.exists("../../data/dynamic_wdpa.rds")
#| fig-cap: "Temporal reconstruction for three emblematic protected areas. (a) Ankarafantsika: transition from Réserve Naturelle Intégrale to Parc National with successive boundary extensions. (b) Ambatovaky: historical boundary corrections revealing discrepancies between WDPA and legal sources. (c) Kirindy Mite: complex status history with temporary protection decrees preceding formal designation."
#| fig-width: 10
#| fig-height: 12
#| warning: false
#| message: false
...
examples <- dynamic_wdpa |>
  filter(grepl(paste(example_names, collapse = "|"), NAME, ignore.case = TRUE),
...
#| label: fig-map
#| eval: !expr file.exists("../../data/dynamic_wdpa.rds")
#| fig-cap: "Map of Madagascar's 123 terrestrial protected areas, colored by the number of documented temporal states. Sites with a single state correspond to the WDPA baseline; sites with multiple states reflect boundary modifications, status changes, or zoning amendments documented through the YAML registry."
#| fig-width: 7
#| fig-height: 10
#| warning: false
#| message: false
...
To demonstrate the empirical consequences of boundary accuracy, we compared deforestation statistics computed from WDPA boundaries before and after the December 2024 correction. Using Global Forest Watch tree cover loss data [@hansen_high-resolution_2013] and the mapme.biodiversity R package, cumulative forest loss between 2001 and 2024 was calculated within protected area boundaries from both the November 2024 and January 2025 WDPA snapshots, applying a uniform 30% canopy cover threshold.

@fig-amendments summarizes the types and counts of amendments documented in the registry. @fig-deforestation presents the results of the deforestation differential analysis: among the 49 protected areas with area changes exceeding 5% between versions, boundary errors in the pre-correction WDPA systematically inflated the apparent deforestation rate, because areas erroneously included in protected area perimeters exhibited higher forest loss than genuinely protected zones.

```{r}
#| label: fig-amendments
#| eval: !expr dir.exists("../../data/amendments")
#| fig-cap: "Types and counts of amendments documented in the YAML registry across the 123 protected areas, classified as boundary modifications, status changes, corrections, and secondary zoning additions."
#| fig-width: 7
#| fig-height: 4
#| warning: false
#| message: false
...
yaml_files <- list.files("../../data/amendments", pattern = "\\.yml$",
                         full.names = TRUE)
...
ggplot(amendment_summary, aes(x = reorder(label, -n), y = n)) +
...
#| eval: !expr file.exists("../../data/wdpa_diff.parquet") || nzchar(Sys.getenv("AWS_ACCESS_KEY_ID"))
#| fig-cap: "Deforestation differential analysis for the 49 protected areas with boundary area changes exceeding 5% between the WDPA November 2024 and January 2025 snapshots. The horizontal axis shows the relative change in protected area surface; the vertical axis shows the relative change in cumulative deforestation rate (2001–2024). Points in the lower-left quadrant indicate protected areas where both surface and measured deforestation decreased after correction, consistent with the removal of unprotected land that had experienced higher forest loss."
#| fig-width: 7
#| fig-height: 6
#| warning: false
#| message: false
...
# Read the pre-computed WDPA diff data (produced by BETSAKA wdpa_update_incidence)
# Try local cache first, then S3
local_path <- "../../data/wdpa_diff.parquet"
s3_path <- "s3://projet-betsaka/diffusion/wdpa_compare/wdpa_diff.parquet"

wdpa_diff <- if (file.exists(local_path)) {
  read_parquet(local_path)
} else {
...
ggplot(ap_var, aes(x = area_ha_diff, y = defor_pct_diff)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey50") +
  geom_vline(xintercept = 0, linetype = "dashed", color = "grey50") +
  geom_point(aes(color = area_ha_diff > 0), size = 3, alpha = 0.7) +
  scale_color_manual(
    values = c("TRUE" = "steelblue", "FALSE" = "tomato"),
    labels = c("Area decreased", "Area increased"),
    name = "Area change direction"
  ) +
  labs(
    x = "Relative change in protected area surface (%)",
    y = "Relative change in deforestation rate (%)"
  ) +
  theme_minimal(base_size = 12) +
  theme(
...
This analysis provides direct empirical evidence that boundary errors in protected area databases propagate into biased conservation effectiveness metrics.
