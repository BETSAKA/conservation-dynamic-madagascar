...
The amendment registry (YAML and GeoJSON files) along with the source metadata is deposited on DataSuds, the institutional research data repository of the Institut de Recherche pour le Développement (IRD), under the DOI: <https://doi.org/10.23708/VN2TWC>.

For the purpose of peer review, the fully consolidated datasets (`dynamic_wdpa.gpkg` and `dynamic_wdpa.parquet`) and the source amendment registry are provided via a secure, anonymous institutional drive. Reviewers can instantly and anonymously download the data without any login or password requirements at the following temporary link:
<https://drive.ird.fr/s/cqo3bZr8RSA2LAM>

This dual-hosting arrangement is maintained pending final permission from UNEP-WCMC to publicly redistribute geometries derived from the WDPA baseline. Once permission is secured, the compiled geodata will be integrated directly into the DataSuds repository.
